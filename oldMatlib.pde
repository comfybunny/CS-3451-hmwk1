...
void gtVertex(float x, float y, float z) {
  ...
  ArrayList<gtMatrix> ctmLayer = matrixStack.get(matrixStack.size()-1);
  
  
  gtMatrix multCTM = new gtMatrix();
  for(int o=0; o<4; o++){
    for(int p=0; p<4; p++){
      multCTM.m[o][p] = matrixStack.get(0).get(0).m[o][p];
    }
  }
  
  for(int i = 1; i<matrixStack.size(); i++){
    gtMatrix tempi = new gtMatrix();
    
    for(int o=0; o<4; o++){
      for(int p=0; p<4; p++){
        tempi.m[o][p] = ctmLayer.get(i).m[o][p];
      }
    }
    
    
    gtMatrix tempMultCTM = new gtMatrix();
    tempMultCTM.m = tempi.multiply(multCTM);
    
    for(int g=0; g<4; g++){
      for(int h=0; h<4; h++){
        multCTM.m[g][h] = tempMultCTM.m[g][h];
      }
    }
  }

  //tempLine.v[0] = multCTM.m[0][0]*tempLine.v[0] + multCTM.m[0][1]*tempLine.v[1] + multCTM.m[0][2]*tempLine.v[2] + multCTM.m[0][3];
  //tempLine.v[1] = multCTM.m[1][0]*tempLine.v[0] + multCTM.m[1][1]*tempLine.v[1] + multCTM.m[1][2]*tempLine.v[2] + multCTM.m[1][3];
  //tempLine.v[2] = multCTM.m[2][0]*tempLine.v[0] + multCTM.m[2][1]*tempLine.v[1] + multCTM.m[2][2]*tempLine.v[2] + multCTM.m[2][3];
  ...
  for(int i = ctm.size()-1; i>=0; i--){
    ...
    tempLine.v[0] = tempT.m[0][0]*tempLine.v[0] + tempT.m[0][1]*tempLine.v[1] + tempT.m[0][2]*tempLine.v[2] + tempT.m[0][3];
    tempLine.v[1] = tempT.m[1][0]*tempLine.v[0] + tempT.m[1][1]*tempLine.v[1] + tempT.m[1][2]*tempLine.v[2] + tempT.m[1][3];
    tempLine.v[2] = tempT.m[2][0]*tempLine.v[0] + tempT.m[2][1]*tempLine.v[1] + tempT.m[2][2]*tempLine.v[2] + tempT.m[2][3];
  }
